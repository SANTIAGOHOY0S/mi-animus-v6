...
# Estado de la sesión para transmisiones
...
# Música de ambiente (Ezio's Family)
youtube_html = """
...
"""
with st.sidebar:
    st.caption("🎵 Frecuencia de la Hermandad activada")
...
with st.sidebar.form("atalaya"):
    ...
    if st.form_submit_button("Sincronizar Atalaya"):
        ...
        if lat and lon:
            ...
            # Eliminamos asteriscos y símbolos para que la voz no los lea
            info_audio = re.sub(r'[*_#]', '', info_shaun)
            
            # --- PROCESAMIENTO DE VOZ (ELEVENLABS) ---
            try:
                if "ELEVENLABS_KEY" in st.secrets and "VOICE_ID" in st.secrets:
                    ...
                    # Respaldo gTTS si no hay llaves
