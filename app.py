...
# --- 1. CONFIGURACIÓN DE INTERFAZ (ESTADO ESTABLE V6.2) ---
st.set_page_config(page_title="Animus OS V6.2 - Restaurado", layout="wide", initial_sidebar_state="expanded")
...
def obtener_reporte(ciudad, pais_nombre, tipo_nodo):
    if model:
        try:
            prompt = (
                f"Eres Shaun Hastings de Assassin's Creed. Eres un historiador británico cínico y pedante. "
                f"Dame un reporte sobre {ciudad}, {pais_nombre} ({tipo_nodo}). "
                "Responde en ESPAÑOL. Sé arrogante y menciona a Abstergo. Mínimo 2 párrafos."
            )
            response = model.generate_content(prompt)
            return response.text if response.text else "Error de enlace."
...
# --- 4. RENDERIZADO DEL MAPA ESTABLE ---
...
# MAPA ESTÁTICO (NO WRAP) PARA EVITAR EL GLITCH
m = folium.Map(
