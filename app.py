...
soundtracks = [
    "C_n-EcznZpE", "d5F9X6qeXco", "NVsSrJJIzDM", 
    "RwDQZI_NRHA", "nyQEQM0CEBQ", "NEpjh30DLas", "PDVnsHC3ypQ"
]

# Usamos % s para evitar el conflicto de llaves en f-strings
musica_html = """
<div id="player"></div>
<script>
...
with st.sidebar.form("atalaya"):
    ...
    if st.form_submit_button("Sincronizar Atalaya"):
        ...
        if lat_manual and lon_manual:
            ...
            except: st.error("Coordenadas inválidas.")
...
for _, f in df.iterrows():
    ...
    # Arreglado el formato de f-string para el HTML del popup
    popup_html = f"""<div style="font-family: monospace; color: {color}; background: #000; padding: 10px; border: 1px solid {color};">
               <b>{f['Tipo']}: {f['Nombre']}</b><br><br>{f['Info']}</div>"""
    folium.Marker([f['Lat'], f['Lon']], popup=folium.Popup(popup_html, max_width=300), icon=folium.Icon(color=color)).add_to(mapa)
